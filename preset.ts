...
Preset.group((preset) => {
	preset
		.extract("src/routes/_global.pcss")
		.if((preset) => [SNOWPACK_SVELTEKIT].includes(preset.context[SETUP]));
	preset
		.extract("src/global.postcss")
	...
	preset
		.extract("src/app.postcss")
		.if((preset) => [VITE_SVELTEKIT].includes(preset.context[SETUP]));
}).withTitle("Adding global PostCSS stylesheet");

Preset.group((preset) => {
	preset
		.extract("src/routes/__layout.svelte")
		.whenConflict("skip")
		.if((preset) =>
			[VITE_SVELTEKIT, SNOWPACK_SVELTEKIT].includes(preset.context[SETUP])
		);
	const GLOBAL_CSS = "__PLACEHOLDER__GLOBAL_CSS__";
	const APP_CSS = "../app.css";
	preset
		.edit("src/routes/__layout.svelte")
		.update((content) => content.replace(GLOBAL_CSS, "./_global.pcss"))
		.if((preset) => [SNOWPACK_SVELTEKIT].includes(preset.context[SETUP]));
	preset
		.edit("src/routes/__layout.svelte")
		.update((content) =>
...
Preset.group((preset) => {
	preset
		...
		.if((preset) =>
			[SNOWPACK_SVELTEKIT, SNOWPACK].includes(preset.context[SETUP])
		);
...
Preset.edit("snowpack.config.cjs")
	.update((content) => {
		let result = content;
...
Preset.group((preset) => {
	preset
		.extract("svelte.config.cjs")
		.whenConflict("skip")
		.withTitle("Adding `svelte.config.cjs`")
		.if((preset) => [VITE].includes(preset.context[SETUP]));
	preset
	...
	preset
		...
		.update((content) => {
			...
			result = result.replace(
				matchEmptyPreprocess,
				(_match) => `[${newPreprocessor}]`
			);
			...
			result = result.replace(
				...
				(_match, otherPreprocessors) => {
					if (otherPreprocessors.includes("preprocess("))
						return addPreprocessor("");
					return addPreprocessor(otherPreprocessors);
				}
			);

			if (!result.includes("svelte-preprocess"))
				result = `const preprocess = require("svelte-preprocess");\n${result}`;
			if (!result.includes("preprocess("))
				result = result.replace(
					"module.exports = {",
					`module.exports = {\n\t${addPreprocessor("")},`
				);
	...
	preset
		...
		.update((content) => {
			...
			if (!result.includes("svelte-preprocess"))
				result = `import preprocess from "svelte-preprocess";\n${result}`;
			if (!result.includes("preprocess("))
				result = result.replace(
					"const config = {",
					`const config = {\n\t${addPreprocessor("")},`
